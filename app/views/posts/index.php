...
<main itemscope itemtype="https://schema.org/Blog">
    ...
    <meta itemprop="description" content='<?= htmlspecialchars($export['description']) ?>'>
    ...
    <div class="post_preview" itemscope itemtype="https://schema.org/Article" data-url="<?= htmlspecialchars($url) ?>/<?= htmlspecialchars($post['url']) ?>.html" data-id="<?= htmlspecialchars($post['url']) ?>">
        ...
        <div class="post_preview_bookmark">
            <a target="_blank" href="/<?= htmlspecialchars($post['url']) ?>.html">№ <?= htmlspecialchars($post['id']) ?></a>
        </div>
        ...
        <div class="post_preview_date_category">
            ...
            <img src="<?= asset('pic/menu/' . htmlspecialchars($post['category_url']) . '.png') ?>" alt="<?= $post['category_name'] ?>" class="icon">
            <span class="spacer_small"></span> <!-- Промежуток 10px -->
            <a href="/cat/<?= $post['category_url'] ?>" class="text_link" itemprop="articleSection"><?= $post['category_name'] ?></a>
        </div>

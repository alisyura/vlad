...
# Тест проверяет ИНТЕГРАЦИЮ между:
# 1. HTTP клиент (Python requests)
# 2. Веб-сервер (Apache/Nginx)
# 3. PHP приложение
# 4. Сессии/Cookies механизм
# 5. Кэширование (X-Cache заголовки)

# API должен соблюдать HTTP спецификацию:
# - HEAD не должен возвращать тело
# - Статус коды должны быть одинаковыми
# - Заголовки должны быть согласованными
